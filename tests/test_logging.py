...
def test_warning_and_error_counting():
    ...
    logger._increment_warnings()
    assert logger.get_warning_count() == 1
    assert logger.get_error_count() == 0
    logger._increment_errors()
    assert logger.get_warning_count() == 1
...
def test_log_contains_essential_info(capsys):
    ...
    logger.log_violation(rule.LogType.ERROR, pos, "a message 2", False,
                         "afilename2.txt", "myrulepack.rule21",
                         ["source line 1", "source line 2"])
    captured = capsys.readouterr()
...
def test_log_hash(capsys):
    ...
    logger.log_violation(rule.LogType.WARNING, pos, "a message", include_white_space,
                         "afilename.txt", "myrulepack.ruleC", ["a line of text"])
