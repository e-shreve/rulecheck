...
class rule_with_settings(rule.Rule):
    ...
    def __init__(self, settings):
        ...
        try:
            ...
            if re.search(r"\s", self.word_to_find):
                raise ValueError("word_to_find setting for rule_with_settings must be a " + \
                                 "single word (no whitespace allowed).")
        except Exception as exc:
            # If a required rule is not provided a KeyError should be raised
            # with a message stating which setting was missing.
            raise KeyError('Settings for rule_with_settings must include word_to_find parameter.') \
                from exc
    ...
    def visit_file_line(self, pos:rule.LogFilePosition, line:str):
        ...
        index_of_word = line.find(self.word_to_find)
        if index_of_word >= 0:
            ...
            # a zero-based offset.
            pos.col = index_of_word + 1
            self.log(rule.LogType.WARNING, pos,
                     "use of the word '" + self.word_to_find + "' found: " + line.rstrip())
